    ...
    <Content>
      ...
      <footer class="h-8 flex items-center justify-center px-5 bg-white dark:bg-[#090d17]/90 border-t border-gray-100 dark:border-[rgba(0,229,255,0.2)] shrink-0 transition-colors duration-300">
        <span class="text-xs text-gray-300 dark:text-[#3a5070]">
          © {{ currentYear }} MindHarness · Powered by Spring AI & Vue 3
        </span>
      </footer>
    </Content>
...
onMounted(() => {
  const saved = localStorage.getItem("mindharness-theme");
  if (saved !== null) {
...
watch(isDark, (val) => {
  ...
  localStorage.setItem("mindharness-theme", val ? "dark" : "light");
}, { immediate: true });
